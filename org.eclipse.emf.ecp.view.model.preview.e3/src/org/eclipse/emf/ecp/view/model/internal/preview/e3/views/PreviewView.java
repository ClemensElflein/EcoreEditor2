	...
	public void createPartControl(Composite parent) {
		...
		for (final IEditorReference reference : PlatformUI.getWorkbench().getActiveWorkbenchWindow().getActivePage()
			...
			if (page.isPartVisible(part)) {
				if (ViewModelEditor.class.isInstance(part)) {
		...
		partListener = new IPartListener2() {
			...
			public void partActivated(IWorkbenchPartReference partRef) {
				if (ViewModelEditor.class.isInstance(partRef.getPart(true))) {
			...
			public void partDeactivated(IWorkbenchPartReference partRef) {
				...
				if (ViewModelEditor.class.isInstance(part) || PreviewView.class.isInstance(part)) {
					removeAdapters();
				}
			}
